...
- Tournament date is not in the PDF: set `EVENT.startsAt` in
  `web/src/lib/event.ts` (and `dateLabel`/`kicker` if wanted) once announced;
  the countdown reappears automatically.
